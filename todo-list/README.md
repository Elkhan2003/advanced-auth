...
let failedQueue: Array<{   // Очередь неудачных запросов
...
const useCreateTodoMutation = () => {
  ...
  return useMutation<TODO.CreateTodoRes, Error, TODO.CreateTodoReq>({
    ...
    onSuccess: (newTodo) => {
      ...
      queryClient.setQueryData(["todos"], (oldData: TODO.GetTodosRes | undefined) => {
...
:root {
  ...
  // Background colors
  --bg-gradient: linear-gradient(135deg, #1a0e3d 0%, #2d1b69 25%, #4a2c96 50%, #6b3cc3 75%, #8b5cf6 100%);
